...
use super::{DbValue, DbValueInto, DeserError};
          ...
          <<RS as DesrlResultset>::ROW as DesrlRow>::V: DbValue
{
    pub fn new(mut rs: RS) -> Result<RsDeserializer<RS>, DeserError> {
        ...
        let rs_struct = RsDeserializer::get_struct(&mut rs)?;
    ...
    fn get_struct(rs: &mut RS) -> Result<RsStructure, DeserError> {
        Ok(match rs.has_multiple_rows()? {
            true => {
    ...
    where <<RS as DesrlResultset>::ROW as DesrlRow>::V: DbValue
{
        ...
        where V: serde::de::Visitor<'x>
    {
        ...
        match self.rows_treat {
            MCD::Done => {
                Err(DeserError::BadStructure("deserialize_seq() when rows_treat = MCD::Done"
                    .to_string()))
            }
            _ => {
    ...
    fn deserialize_newtype_struct<V>(self, _name: &'static str, visitor: V) -> DeserResult<V::Value>
        where V: serde::de::Visitor<'x>
    {
        trace!("RsDeserializer::deserialize_newtype_struct() called with _name = {}",
               _name);
        ...
        where V: serde::de::Visitor<'x>
    {
        match self.next_key {
            Some(i) => {
                ...
                trace!("RsDeserializer::deserialize_identifier(): column {:?} ({})",
                       i,
                       fieldname);

                let row = self.rs.last_row().unwrap();
                trace!("Current row: {:?}", row);
...
impl<'x, 'a, R: DesrlResultset> serde::de::MapAccess<'x> for FieldsVisitor<'a, R> {
    ...
    {
        trace!("FieldsVisitor::next_key_seed() called");
        match self.de.last_row_length().unwrap() {
            0 => {
                trace!("FieldsVisitor::next_key_seed() called on empty row");
                Ok(None)
            }
            len => {
                let idx = len - 1;
                trace!("FieldsVisitor::next_key_seed() for column {}", idx);
    ...
    {
        match self.de.last_row_length().unwrap(/* FIXME */) {
            0 => Err(prog_err("FieldsVisitor::next_value_seed(): no more value")),
            len => {
                trace!("FieldsVisitor::next_value_seed() for column {}", len - 1);
